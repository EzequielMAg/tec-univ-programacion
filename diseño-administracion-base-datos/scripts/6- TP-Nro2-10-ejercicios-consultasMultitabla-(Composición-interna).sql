...
SELECT DISTINCT 
  d.codigo, 
  d.nombre, 
  (d.presupuesto - d.gastos) AS "presupuesto disponible"
FROM Empleado e
INNER JOIN Departamento d 
ON e.codigo_departamento = d.codigo;
...
SELECT d.codigo, d.nombre, e.codigo, e.nombre, e.nif  
FROM Empleado e 
INNER JOIN Departamento d 
ON e.codigo_departamento = d.codigo 
WHERE e.nif = "38382980M"
...
-- Lo anterior es lo que exactamente pide el ejercicio, pero quiero ver mas info!
SELECT d.codigo, d.nombre, e.codigo, e.nombre, e.apellido1, e.apellido2  
FROM Empleado e 
INNER JOIN Departamento d 
ON e.codigo_departamento = d.codigo 
WHERE concat(e.nombre, " ", apellido1, " ", apellido2) = "Pepe Ruiz Santana";
